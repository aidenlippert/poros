...
def init_agent_background():
    ...
    agent = Agent(
        ...
        key_file="greeting_assistant_chat.key",
        demo_mode=True
    )
...
def process_user_query(query: str):
    """Process user query and determine what to do"""
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
        ...
        try:
            ...
            if any(word in query_lower for word in ["day", "date", "today", "what day"]):
                st.session_state.messages.append({
                    "role": "assistant",
                    "content": "📅 Let me check with the Day Agent..."
                })
            ...
            # Check for greeting request
                ...
                if name_match:
                    ...
                    # Try to get name from simple "hello [name]" pattern
                    words = query.split()
                    if len(words) >= 2:
                ...
                st.session_state.messages.append({
                    "role": "assistant",
                    "content": f"💬 Creating personalized greeting for **{name}**..."
                })
                ...
                st.session_state.messages.append({
                    "role": "assistant",
                    "content": f"✅ Day Agent says: {current_day}"
                })
    ...
    result = loop.run_until_complete(handle_query())
    loop.close()
    return result
